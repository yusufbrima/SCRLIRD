...
class Trainer:
    ...
    def finetune(self, d_input_dir = config.filepaths['base_path'],d_output =  None, n_augmentations=1,\
        ...
        wr.split_dataset()
        for i in range(0,n_augmentations):
            ts = time.time()
            ...
            if i == 0:
                pp.build_ds(ds_bld.augmentations[i], keepdims=True,augment = True,random_augmentation=True,n_augmented_samples = i, crop_dims= (128,128),outpath=train_file_path)
            else:
                pp.build_ds(ds_bld.augmentations[i], keepdims=True,augment = True,random_augmentation=True,n_augmented_samples = i, crop_dims= (128,128),outpath=train_file_path)
            ...
            # df =  pp.get_files()
            if i == 0:
                pp.build_ds(ds_bld.augmentations[i], keepdims=True,augment = False,random_augmentation=False,n_augmented_samples = i, crop_dims= (128,128),outpath=val_file_path)
            # print(df.shape)
        ...
        #     # We are preprocessing the test dataset
            ...
            if i == 0:
                pp.build_ds(ds_bld.augmentations[i], keepdims=True,augment = False,random_augmentation=False,n_augmented_samples = i, crop_dims= (128,128),outpath=test_file_path)
            ...
            if save:
                classifier.save(f"{Path(config.filepaths['model_path'], f'AugmentationSize/{i}/{self.m_name}_Baseline_Classifier_Chimp_test')}")
                logging.info(f"Classifier saved to {Path(config.filepaths['model_path'], f'AugmentationSize/{i}/{self.m_name}_{type}_encoder_test')} successfully")
            # The next section is to fine tune the pre-trained models using the downstream dataset
            if len(self.pt_models)  > 0 :
                for j in range(len(self.pt_models)):
                    logging.info(self.pt_models_names[j])
                    #Contrastive Learning phase for infoNCE fine-tuning
                    classifier = self.pt_base_constructor[j].create_classifier(self.pt_models[j], trainable=False, num_classes= len(train_dl.CLASSES))
                    classifier.fit(x=X, y=y, batch_size=batch_size, epochs=epochs, validation_data=(val_dl.X, val_dl.y) )
                    top_1_accuracy,top_3_accuracy = classifier.evaluate(test_dl.X, test_dl.y)[1:]
                    print(f"Test accuracy {self.pt_models_names[j]} after : {round(top_1_accuracy * 100, 2)}%, Top 3 accuracy {round(top_3_accuracy * 100, 2)}%")
                    if j == 0:
                    ...
                    if save:
                        classifier.save(f"{Path(config.filepaths['model_path'], f'AugmentationSize/{i}/{self.pt_models_names[j]}_Classifier_test')}")
                        logging.info(f"Classifier saved to {Path(config.filepaths['model_path'], f'AugmentationSize/{i}/{self.m_name}_{self.pt_models_names[j]}_encoder_test')} successfully")
            results['aug'].append(i)
            ts = time.time()
            dt = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
            ...
            logging.info(rdf)
            rdf.to_csv(Path(f"./Data/Pretrained_multimodel_{self.m_name}_{args.d_output}.csv"))
            logging.info(Path(f"./Data/Pretrained_multimodel_{self.m_name}_{args.d_output}.csv"))
    ...
    def pretrain(self, loss_fn, dl_pt, save=False, bs = config.learningparams['batch_size'], epochs = config.learningparams['num_epochs'], m_type=["InfoNCE", "Triplet"]) -> None:
        ...
        self.pt_base_constructor = []
        for i in range(len(loss_fn)):
            self.scrl = SCRL(dl_pt.input_shape,len(dl_pt.CLASSES),model=self.model)
            self.encoder = self.scrl.create_encoder()
            self.encoder_with_projection_head = self.scrl.add_projection_head(self.encoder)
            ...
            self.pt_models.append(self.encoder)
            self.pt_models_names.append(m_type[i])
            self.pt_base_constructor.append(self.scrl)
            if save:
                self.encoder_with_projection_head.save(f"{Path(config.filepaths['model_path'], f'{self.m_name}_{m_type[i]}_encoder_test')}")
                logging.info(f"Encoder saved to {Path(config.filepaths['model_path'], f'{self.m_name}_{m_type[i]}_encoder_test')} successfully")
...
if __name__ == "__main__":
    ...
    if args.command  == 'build':
        ...
        trainer.pretrain(loss_fn=loss_fns, dl_pt= dl2, save=False, bs = args.pbs, m_type=loss_fn_names, epochs=args.epochs)
        if args.ft == 1:
            logging.info("Here we are fine-tuning the pre-trained model")
            print(trainer.encoder_with_projection_head.summary())
